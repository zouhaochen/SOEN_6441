...
public class TournamentResultsView {
    ...
    public static void printTournamentResults(TournamentController p_TournamentController) {
        ...
        int l_MapLoop = 0;
        for (int i = 0; i < p_TournamentController.getMapFiles().size(); i++) {
            System.out.format("%15s", p_TournamentController.getMapFiles().get(i));

            for (int j = 0; j < p_TournamentController.getNumberOfGames(); j++) {
