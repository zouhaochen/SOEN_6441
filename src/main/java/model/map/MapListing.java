...
/**
 * This class is to get some lists and correspondence from the model.map file
...
public class MapListing extends MapDetailAccess {
     ...
     *
     * @param p_File contains the file path of a model.map
        ...
        while (l_Sc.hasNextLine()) {
            ...
            if (l_Search.equals("[continents]")) {
                for (l_I = 0; l_I < l_continentnumber; l_I++) {
                    ...
                    // Get only the name of the continent in the model.map file.
        ...
        while (l_Sc.hasNextLine()) {
            ...
            if (l_Search.equals("[countries]")) {
                for (l_I = 0; l_I < l_Countrynumber; l_I++) {
                    ...
                    // Get only the name of the country in the model.map file.
                    String l_Text = l_Sc.nextLine();
                    String[] l_A = l_Text.split(" ");
                    l_List.add(l_A[1]);
                }
    ...
    public HashMap<String, String> getCountryContinent(File p_File) {
        ...
        int l_J = 0;
        int l_Countrynumber = getCountryNumber(p_File);
        // To get the location of [countries] in the model.map file.
        while (l_Sc.hasNextLine()) {
            if (l_J == 1) {
                break;
            }
     ...
     *
     * @param p_File   This is the model.map file.
