...
public class CommandValidator {
	...
	public boolean validate(String p_Command) {
		...
		switch (d_GameData.getCurrentPhase()) {
			case MAP_EDIT:
				l_Valid = validateMapCommands(l_Command_arr);
				break;
			case STARTUP:
	 ...
	 *
	 * @param p_Command_arr the command from console
	 * @return true if the command is valid, false otherwise
	 */
	private boolean validateIssueOrderCommands(String[] p_Command_arr) {
		String l_CommandType = p_Command_arr[0].toLowerCase();
		boolean l_Result;
		switch (CommandType.getCommandFromLabel(l_CommandType)) {
			case SHOW_MAP:
				l_Result = validateNoArgumentCommand(p_Command_arr);
				break;
			case DEPLOY:
				l_Result = validateDeploy(p_Command_arr);
				break;
			default:
	...
	private boolean validateStartupCommands(String[] p_Command_arr) {
		...
		switch (CommandType.getCommandFromLabel(l_CommandType)) {
			case LOAD_MAP:
				l_Result = validateLoadMap(p_Command_arr);
				break;
			case ADD_PLAYER:
				l_Result = validateAddPlayer(p_Command_arr);
				break;
	 ...
	 */
	private boolean validateNoArgumentCommand(String[] p_Command_arr) {
		if (p_Command_arr.length != 1) {
	 ...
	 */
	private boolean validateLoadMap(String[] p_Command_arr) {
		if (p_Command_arr.length != 2) {
			printInvalidArguments();
			return false;
		}

		String l_FileName = p_Command_arr[1];
	 ...
	 */
	private boolean validateAddPlayer(String[] p_Command_arr) {
		if (p_Command_arr.length < 3 || p_Command_arr.length % 2 != 1) {
		...
		for (int l_Index = 1; l_Index < p_Command_arr.length; ) {
			if (!p_Command_arr[l_Index].equalsIgnoreCase(l_AddTag) && !p_Command_arr[l_Index].equalsIgnoreCase(l_RemoveTag)) {
				printInvalidArguments();
				return false;
			}
			l_Index += 2;
